...
class Compressor(nn.Module):
    def __init__(
        ...
        self.dims = [channels, *map(lambda m: dim * m, dim_mults)] # [1, 32, 64]
        self.in_out = list(zip(self.dims[:-1], self.dims[1:])) # [(1, 32), (32, 64)]
        self.reversed_dims = [*map(lambda m: dim * m, reverse_dim_mults), out_channels] # [64, 32, 32]
        self.reversed_in_out = list(zip(self.reversed_dims[:-1], self.reversed_dims[1:])) # [(64, 32), (32, 32)]
        assert self.dims[-1] == self.reversed_dims[0]
        self.hyper_dims = [self.dims[-1], *map(lambda m: dim * m, hyper_dims_mults)] # [64, 64, 64]
        self.hyper_in_out = list(zip(self.hyper_dims[:-1], self.hyper_dims[1:])) # [(64, 64), (64, 64)]
...
class ResnetCompressor(Compressor):
    def __init__(
        ...
        super().__init__(
            dim, # 32
            dim_mults, # [1,2]
            reverse_dim_mults, # [2,1]
            hyper_dims_mults, # [2,2,2]
            channels, # 2
            out_channels # 32
        )
        self.build_network()
